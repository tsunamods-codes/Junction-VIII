...
namespace Iros.Workshop
{
    public static class Install
    {
        ...
        public static void DownloadAndInstall(Mod m, bool isUpdatingMod = false)
        {
            ...
            string file = String.Format("{0}_{1}_{2}.iroj", m.ID, SafeStr(m.Name), m.LatestVersion.Version);
            ...
            if (install != null)
            {
                ...
                if (patches.Any())
                {
                    ...
                    string pfile = String.Format("{0}_{1}_{2}.irojp", m.ID, SafeStr(m.Name), m.LatestVersion.Version);
                ...
                {
                    // no patches available to update so download entire new mod version .iroj
            ...
            if (m.Patches.Any())
            {
                ...
                {
                    string pfile = String.Format("{0}_{1}_{2}_patch{3}.irojp", m.ID, SafeStr(m.Name), m.LatestVersion.Version, pCount);
        ...
        internal class InstallModProcedure : InstallProcedure
        {
                    ...
                    else
                    {
                        using (var fs = new FileStream(source, FileMode.Open, FileAccess.Read))
                        {
                            ...
                            else
                            {
                                ...
                                if (_dest.EndsWith(".iroj", StringComparison.InvariantCultureIgnoreCase)) _dest = _dest.Substring(0, _dest.Length - 4);
            ...
            public override void DownloadComplete(System.ComponentModel.AsyncCompletedEventArgs e)
            {
                ...
                bool isIro = Path.GetExtension(_dest) == ".iroj";
