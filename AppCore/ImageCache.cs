    ...
    public class ImageCache {
        ...
        public string GetImagePath(string url, Guid modID)
        {
            ...
            if (gotValue && e.File != null)
            {
                ...
                if ((!File.Exists(file) || e.Updated < DateTime.Now.AddDays(-5)) && !url.StartsWith("iroj://Preview/Auto", StringComparison.InvariantCultureIgnoreCase))
                {
                    TriggerDownload(url, modID);
                }
            }
            else if (!url.StartsWith("iroj://Preview/Auto", StringComparison.InvariantCultureIgnoreCase))
            {
                TriggerDownload(url, modID);
            }
