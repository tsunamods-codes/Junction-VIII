...
namespace AppCore
{
    ...
    public static class RegistryHelper
    {
        ...
        public const string FF8SteamKeyPath64Bit = @"HKEY_CURRENT_USER\Software\Wow6432Node\Valve\Steam\Apps\39150";
        ...
        public const string FF8AppKeyPath64Bit = @"HKEY_LOCAL_MACHINE\SOFTWARE\Wow6432Node\Microsoft\Windows\CurrentVersion\App Paths\FF8.exe";

        public const string FF8AppKeyPath32Bit = @"HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\App Paths\FF8.exe";
        ...
        /// </summary>
        public static string GetKeyPath(FF8RegKey regKey)
        {
        ...
        /// </summary>
        public static string GetKeyPath(FF8RegKey regKey, bool use32BitKey)
        {
            if (regKey == FF8RegKey.SteamKeyPath)
            {
                if (use32BitKey)
                {
            ...
            if (regKey == FF8RegKey.FF8AppKeyPath)
            {
                if (use32BitKey)
                {
                    return FF8AppKeyPath32Bit;
                }
                else
                {
                    return FF8AppKeyPath64Bit;
                }
            }

            if (regKey == FF8RegKey.SquareSoftKeyPath)
            {
                if (use32BitKey)
                {
            ...
            if (regKey == FF8RegKey.VirtualStoreKeyPath)
            {
                if (use32BitKey)
                {
        ...
        public static object GetValue(FF8RegKey key, string valueName, object defaultVal = null)
        {
        ...
        public static void CommitTransaction()
        {
            ...
            if (transaction.Count > 0)
            {
                ...
                {
                    Sys.Message(new WMessage() { Text = $"Error while trying to update registry settings", LoggedException = e });
                }
            }
