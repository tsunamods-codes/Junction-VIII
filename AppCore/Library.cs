...
namespace Iros.Workshop
{
    ...
    {
        /// <summary>
        /// Dictionary to cache ModInfo instances during runtime. This is to avoid having to open .iroj or mod.xml file from disk everytime
        /// </summary>
        [System.Xml.Serialization.XmlIgnore]
        private static Dictionary<string, ModInfo> _infoCache = new Dictionary<string, AppWrapper.ModInfo>(StringComparer.InvariantCultureIgnoreCase);
        ...
        public ModInfo GetModInfo()
        {
            ...
            try
            {
                ...
                {
                    if (pathToModFileOrFolder.EndsWith(".iroj", StringComparison.InvariantCultureIgnoreCase))
                    {
    ...
    public class InstalledVersion
    {
        ...
        public Stream GetData(string name)
        {
            ...
            if (InstalledLocation.EndsWith(".iroj", StringComparison.InvariantCultureIgnoreCase) && File.Exists(path))
            {
                using (var arc = new AppWrapper.IrosArc(path))
