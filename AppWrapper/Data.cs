...
namespace ProcMonParser {
    ...
    public class DataFile {
        ...
        public DataItem Get(int offset) {
            ...
            if (min > 0)
            {
                AppWrapper.DebugLogger.WriteLine("Mid read from " + Filename + " at offset " + offset);
            }
    ...
    public static class FF8Files {
        ...
        public static DataFile LoadSounds(string ff8folder) {
            int count = 0;
            DataFile df = new DataFile() { Filename = System.IO.Path.Combine(ff8folder, "sound\\audio.dat") };
            using (var fmtFile = new System.IO.FileStream(System.IO.Path.Combine(ff8folder, "sound\\audio.fmt"), System.IO.FileMode.Open, System.IO.FileAccess.Read, System.IO.FileShare.Read)) {
