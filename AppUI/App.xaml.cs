...
namespace AppUI
{
    ...
    public partial class App : Application
    {
        ...
        internal static void ProcessCommandLineArgs(string[] args, bool closeAfterProcessing = false)
        {
            ...
            {
                if (parm.StartsWith("iroj://", StringComparison.InvariantCultureIgnoreCase))
                {
                    // check if its a catalog url or a mod download url
                    if (parm.StartsWith("iroj://mod/"))
                    {
                        ...
                        App.Current.Dispatcher.Invoke(() =>
                        {
                            Mod newMod = new Mod()
                            {
                                ...
                                {
                                    Links = new System.Collections.Generic.List<string>() { parm.Replace("iroj://mod/", "iroj://") },
                                    Version = 1,
                                },
                            };
                ...
                else if (parm.Equals("/QUIT", StringComparison.InvariantCultureIgnoreCase))
                {
                    if (hasLaunchCommand)
                    {
                        ...
                        while (isGameLaunching || GameLauncher.IsFF8Running())
                        {
                            Thread.Sleep(10000); // sleep 10 seconds and check again until ff8 not running
                        ...
                        // wait for ff8 to close before closing down our app
                    }
                ...
                else if (parm.StartsWith("/PACKIRO:", StringComparison.InvariantCultureIgnoreCase))
                {
                    ...
                    try
                    {
                        ...
                        PackIroViewModel packViewModel = new PackIroViewModel()
                        {
                            ...
                            PathToOutputFile = Path.Combine(parentDir, $"{folderName}.iroj"),
                        };
        ...
        internal static string GetAppName()
        {
            ...
            return "Junction VIII"; // default if can't find for some reason
        }
