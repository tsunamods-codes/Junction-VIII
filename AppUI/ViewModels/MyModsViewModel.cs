...
namespace AppUI.ViewModels
{
    ...
    public class MyModsViewModel : ViewModelBase
    {
        ...
        /// <summary>
        /// Loop over installed mods and update Cached Details of mod by parsing mod.xml (if mod is installed in folder format, not .iroj file)
        /// </summary>
        ...
        internal void ShowConfigureModWindow(InstalledModViewModel modToConfigure)
        {
            ...
            AppWrapper.ModInfo info = modToConfigure.InstallInfo.GetModInfo();
            if (info == null)
            {
            ...
            // wire up the imageReader and audioReader to extract/read files from the .iroj archive
            IDisposable arcToDispose = null;
            if (pathToModXml.EndsWith(".iroj", StringComparison.InvariantCultureIgnoreCase))
            {
            ...
            using (arcToDispose)
            {
                info = info ?? new AppWrapper.ModInfo();
        ...
        public void AutoSortBasedOnCategory()
        {
            ...
            {
                AppWrapper.ModInfo info = installedMod.InstallInfo.GetModInfo();
