    ...
    public partial class GameLaunchWindow : Window
    {
        ...
        private Task<bool> StartGameLaunchProcessAsync()
        {
            ...
            task.ContinueWith((taskResult) =>
            {
                ...
                {
                    ViewModel.LogStatus(ResourceHelper.Get(StringKey.FailedToLaunchFf8ViewLogForDetails));
                    ShowOkButton();
                }
                else
                {
                    Sys.Message(new WMessage(ResourceHelper.Get(StringKey.SuccessfullyLaunchedFf8)));
