...
begin
  try
    IsProductInstalled := Dependency_IsNetCoreInstalled('Microsoft.WindowsDesktop.App 10.0.11')
  except
  ...
  if not IsProductInstalled then begin
    Dependency_Add('dotnet100desktop' + Dependency_ArchSuffix + '.exe',
      ...
      Dependency_String('https://builds.dotnet.microsoft.com/dotnet/WindowsDesktop/10.0.11/windowsdesktop-runtime-10.0.11-win-x86.exe', 'https://builds.dotnet.microsoft.com/dotnet/WindowsDesktop/10.0.11/windowsdesktop-runtime-10.0.11-win-x64.exe'),
      '', False, False);
  end;
end;
